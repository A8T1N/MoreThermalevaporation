...
public class UltimateThermalEvaporationValidator extends CuboidStructureValidator<UltimateThermalEvaporationMultiblockData> {
    ...
    @Override
    protected FormationResult validateFrame(FormationProtocol<UltimateThermalEvaporationMultiblockData> ctx, BlockPos pos, BlockState state, CasingType type, boolean needsFrame) {
        boolean controller = structure.getTile(pos) instanceof TileEntityUltimateThermalEvaporationController;
    ...
    protected CasingType getCasingType(BlockState state) {
        ...
        if (BlockType.is(block, MoreThermalEvaporationBlockTypes.ULTIMATE_THERMAL_EVAPORATION_BLOCK)) {
            return CasingType.FRAME;
        } else if (BlockType.is(block, MoreThermalEvaporationBlockTypes.ULTIMATE_THERMAL_EVAPORATION_VALVE)) {
            return CasingType.VALVE;
        } else if (BlockType.is(block, MoreThermalEvaporationBlockTypes.ULTIMATE_THERMAL_EVAPORATION_CONTROLLER)) {
    ...
    public boolean precheck() {
        int maxHeight = MoreThermalEvaporationConfig.UltimateEvaporationPlantHeight.get();
    ...
    @Override
    public FormationResult postcheck(UltimateThermalEvaporationMultiblockData structure, Long2ObjectMap<ChunkAccess> chunkMap) {
        if (!foundController) {
